// src/components/GrammarPDFPage.jsx
import React, { useRef, useEffect, useState } from 'react';
import GenieImage from '../assets/Genie.png';

export default function GrammarPDFPage({ unit, onBack, isDarkMode }) {
  const [sidebarInput, setSidebarInput] = useState("");
  const [sidebarMessages, setSidebarMessages] = useState([
    { text: "Hello! Ask me anything about this unit 📘", isUser: false }
  ]);
    ...
    <div className="flex h-screen">
      ...
      <div className="flex-1 overflow-auto p-4 bg-white">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-2xl font-bold text-[#7030a0]">Unit {unit} Grammar PDF</h2>
          <button onClick={onBack} className="px-4 py-2 bg-[#7030a0] text-white rounded-lg hover:bg-[#512b81]">← Back</button>
        </div>
        <iframe
         src={`/grammar_pdfs/Unit_${unit}_Grammar_PDF.pdf`}
          title={`Unit ${unit} PDF`}
          className="w-full"
          style={{ height: '90vh' }}
        ></iframe>
